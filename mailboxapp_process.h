...
extern int mailboxapp_init(char *process_name, key_t key, int wsize, int delay, int to, int drop);
extern int mailboxapp_get_process_info(char *process_name, process_t *info);
extern int mailboxapp_get_num_packets(char *data, int packet_size);
extern int mailboxapp_create_packets(char *data, message_status_t *message_stats);
extern int mailboxapp_get_next_packet(int num_packets);
extern int mailboxapp_send_packet(packet_t *packet, int mailbox_id, int pid);
extern int mailboxapp_drop_packet();
extern int mailboxapp_send_message(char *receiver, char* content);
extern int mailboxapp_send_ACK(int mailbox_id, pid_t pid, int packet_num);
extern int mailboxapp_receive_message(char *data);
extern void mailboxapp_receive_packet(int sig);
